...
final class HivemallOps(df: DataFrame) extends Logging {
   ...
   */
  @scala.annotation.varargs
  def train_xgboost_regr(exprs: Column*): DataFrame = {
     Generate(HiveGenericUDTF(
        new HiveFunctionWrapper("hivemall.xgboost.regression.XGBoostRegressionUDTF"),
        setMixServs(exprs: _*).map(_.expr)),
      join = false, outer = false, None,
   ...
   */
  @scala.annotation.varargs
  def train_xgboost_classifier(exprs: Column*): DataFrame = {
     Generate(HiveGenericUDTF(
        new HiveFunctionWrapper("hivemall.xgboost.classification.XGBoostBinaryClassifierUDTF"),
        setMixServs(exprs: _*).map(_.expr)),
      join = false, outer = false, None,
      Seq("model_id", "pred_model").map(UnresolvedAttribute(_)),
      df.logicalPlan)
   ...
   */
  @scala.annotation.varargs
  def train_multiclass_xgboost_classifier(exprs: Column*): DataFrame = {
     Generate(HiveGenericUDTF(
        new HiveFunctionWrapper("hivemall.xgboost.classification.XGBoostMulticlassClassifierUDTF"),
        setMixServs(exprs: _*).map(_.expr)),
      join = false, outer = false, None,
      Seq("model_id", "pred_model").map(UnresolvedAttribute(_)),
      df.logicalPlan)
   ...
   */
  @scala.annotation.varargs
  def xgboost_predict(exprs: Column*): DataFrame = {
     Generate(HiveGenericUDTF(
   ...
   */
  @scala.annotation.varargs
  def xgboost_multiclass_predict(exprs: Column*): DataFrame = {
     Generate(HiveGenericUDTF(
