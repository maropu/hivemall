...
public class WorkerProcessBuilderTest {

    @Test
    public void testInvokeProcess() {
        WorkerProcessBuilder builder = new WorkerProcessBuilder(
                LauncherTest.class, 512, Arrays.asList("test"), null);
        String output = null;
        int exitCode = -1;
        try {
            Process process  = builder.execute();
            InputStream input = process.getInputStream();
            output = IOUtils.toString(input);
            exitCode = process.waitFor();
        } catch (Exception e) {
            Assert.fail("Failed to launch a program:" + builder);
        }
